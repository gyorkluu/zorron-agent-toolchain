...
version: 1.1.0
description: "Generate a single-file HTML interactive presentation (PPT) with two premium design styles: 'A: Editorial Magazine' and 'B: Swiss Internationalism'. Features horizontal swipe, WebGL fluid backgrounds, dot grids, Lucide icons, Motion One animations, and automated transition-to-video rendering using Hyperframes. Trigger this skill when the user asks to create, design, or structure a high-end web-based presentation deck or convert it into a dynamic mp4 video. DO NOT invoke this skill if the user only wants a standard office PowerPoint (.pptx) file, a simple Markdown document, or does not need visual slide layouts or video rendering."
---
...
- **Use this skill** when the user requests to create or edit a web-based, interactive presentation (slides/deck) using HTML and CSS.
- **Use this skill** when the user wants to convert a web-based HTML presentation into a dynamic MP4 video with slide transitions and content entry animations.
- **Use this skill** when the user mentions keywords like "Swiss Style PPT", "杂志风 PPT", "horizontal swipe deck", "hyperframes render", or "HTML to video presentation".
- **DO NOT invoke this skill** if the user requests a standard desktop PowerPoint file (`.pptx`), a simple plain-text document, or is not dealing with slide layouts and animation transitions.
...
瑞士风用于投屏演示时,小字不能按网页注释 of 10-12px 写。默认遵守以下下限:
...
20. **底部分页重叠防错**——`.canvas-card` 和 split `.half` 底部 padding 设为 `9.6vh`，`#nav` 设在 `bottom: 3vh`。包含嵌套 grid/flex 的主容器必须显式加 `min-height: 0` 防溢出重叠。
...
## ⚠️ Gotchas & Tricky Details

### Hyperframe 视频渲染与自动化翻页指南及踩坑经验 (Troubleshooting & Lessons Learned)

当使用 `@hyperframes/cli` 或 `hyperframes render` 将生成的 HTML 页面转换为动态视频（.mp4）时，常会遇到视频没有动态动画、页面未自动切换、视频变静态、或在 `file://` 下运行时报 CORS 等问题。以下为完整的实战指南与修复细节：

### 1. 自动翻页与 Timeline 劫持拦截 (Timeline & Player Hijacking)
- **现象**：视频生成后是一张静止的图片（通常卡在第一页），没有自动翻页或动画。
- **原因**：Hyperframe 的渲染文件服务器在发送 HTML 时，会自动在 `</body>` 前注入 `HF_BRIDGE_SCRIPT`（桥接脚本）。此脚本会覆盖 `window.__hf` 和 `window.__timelines` 注册的 mock。
- **解决方案**：
  1. 必须在 `window.__timelines` 注册完整的 timeline 对象，例如：
     ```javascript
     window.__timelines['anthropic-prompt'] = {
       getChildren: () => [],
       pause() {},
       play() {},
       seek(t) {
     ...
     ```
  2. 劫持 `window.__player.renderSeek` 属性。使用 `Object.defineProperty(window, '__player', ...)` 劫持 `__player` 对象的 setter/getter，从而在 runtime 赋值 `__player` 时能够自动替换并劫持其 `renderSeek` 方法，拦截到 seek 信号并手动驱动页面翻页。

### 2. 动效在视频中完全静止的问题 (Preserving Motion.js Animations in Video Mode)
- **现象**：视频有横向切页动画，但页面内的具体动态效果（如文字浮现、卡片生长等 `motion.js` 配方动画）全部缺失，变为了生硬的静态显示。
- **原因**：为了避免 CPU 密集型的 Canvas（如 WebGL/ASCII 点阵背景）拖慢 headless 渲染，先前的绕过手段常常是在 seek 时将 `window.__lowPowerMode = true` 并添加 `body.low-power` 样式。但该样式在 CSS 中会通过 `* { animation: none !important; transition: none !important; }` 强行将 `[data-anim]` 等动画全部抹去，且 `playSlide` 中也会直接进入 `revealStatic(slide)` 状态。
- **解决方案**：
  1. 引入独立的 `window.__hyperframesMode = true` 变量。
  2. 当处于该渲染模式时，既要防止 Canvas 背景动画运行，又要保留 `motion.js` 配方动效运行。可以通过：
     - 不要把 `low-power` class 加到 `body` 上。而是添加自定义 `body.render-video` class，专门通过 CSS 隐藏 Canvas：
       ```css
       body.low-power canvas.bg,
       body.low-power canvas.ascii-bg,
       body.render-video canvas.bg,
       body.render-video canvas.ascii-bg { display: none !important; }
       ```
     - 触发自定义事件 `swiss-low-power-change` 并传参 `{ detail: { on: true } }` 来主动停掉 Canvas background 的 `requestAnimationFrame` 刷新循环。
     - 在 `playSlide(i)` 中将静止判断修改为：`if (window.__lowPowerMode && !window.__hyperframesMode)`，使得视频渲染时能够正常进入 `RECIPES` 动画。

### 3. 帧精准控制与 Web Animations scrubbing (Frame-Perfect Scrubbing)
- **现象**：在 frame-by-frame 视频渲染中，`setTimeout` 异步动画不会跟随 seek 时间，导致捕获的画面中动画状态错乱。
- **解决方案**：
  1. 在 `go(pageIndex)` 页面切换方法中，遇到 `window.__hyperframesMode` 时，必须绕过 `setTimeout`，**同步**且**立即**执行 `window.__playSlide(pageIndex)` 实例化动画。
  2. 在 `transitionToPage(t)` 方法中，通过 `document.getAnimations()` 获取页面中所有 Web Animations，并将其暂停（`.pause()`），同时将它的 `.currentTime` 精确同步到基于时间偏移 `t` 计算的毫秒数上（例如 `animTimeMs = Math.max(0, (t - (slideStart + 0.45)) * 1000)`）。
  3. 横向翻页 `#deck` 容器同样要禁用 CSS transition，并在 `transitionToPage(t)` 里手动计算其贝塞尔曲线插值后设置 `translateX(...)vw`，从而实现帧级的切页效果。
