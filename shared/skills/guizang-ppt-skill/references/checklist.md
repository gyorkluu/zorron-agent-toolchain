...
### 0-S-5. Swiss 纵向时间轴直接子节点对齐 (禁止 tl-body 包装层)

**现象**: 时间轴中的时间、倍率和描述文字被挤在非常窄的区域（1-2个字就换行），右侧大面积空置，而文字痛苦堆叠。

**根因**: `.tl-node` 在 CSS 中被定义为一个 **4列网格** (Grid Layout)，四列分别为 `tl-axis`（24px）、`yr`（7em）、`multi`（7.6em）和 `desc`（`1fr`）。如果把时间、倍数和描述套在一个 `.tl-body` 包装层里，浏览器会将 `.tl-body` 识别为第 2 个子网格元素，强行将其塞入 `7em` 宽的第二列，导致第四列 `1fr` 空间完全被闲置。

**做法**:
- **严禁**使用任何 `.tl-body` 包装层。
- 让 `.yr`、`.multi`、`.desc` 作为 `.tl-node` 的 **直接直属子元素**。
- 正确的 HTML 结构如下：
  ```html
  <div class="tl-node" style="padding:1.8vh 0">
    <div class="tl-axis"><span class="dot"></span></div>
    <span class="yr">08:00</span>
    <span class="multi">3<small>×</small></span>
    <p class="desc"><strong>描述标题</strong>：具体说明内容...</p>
  </div>
  ```

---

### 0-S-6. Swiss 视口高度溢出与导航重叠防错 (矮视口屏必守)

**现象**: 在高度较矮的屏幕（例如 laptop 视口高度 `647px` 或以下）上，底部的文字、KPI描述或卡片会与页码圆点（`#nav`）和右下角快捷提示（`#hint`）垂直重叠，影响观感。

**根因**: 
- 默认底边距较小（如 `4.4vh`），没有专门为页码和提示隔离底栏。
- 含有 `flex: 1` 的弹性子元素（如 display:grid 容器）默认 `min-height` 为 `auto`，在内容高度超过容器高度时会直接撑破父容器，渲染在 `.canvas-card` 的 padding-bottom 区域中。
- 时间轴节点垂直 padding 过大（如 `4.5vh 0`）或卡片 padding 过大导致内容累计高度超限。

**做法**:
- **安全底边距设置**：将 `.canvas-card`、`.image-hero-body` 和 split 的 `.half` 底部 padding 统一调整为 **`9.6vh`**，作为翻页指示器专属隔离层。
- **指示器位置上移**：将 `#nav` 的 `bottom` 改为 **`3vh`**，将 `#hint` 的 `bottom` 改为 **`3.4vh`**，让它们自然居中于底栏安全带。
- **强制收缩限制**：在 `.canvas-card` 下带有 `flex: 1` 且内部为 grid 或 flex-column 的直系主内容容器上，**必须**显式声明 **`min-height: 0`**，强迫内容自适应收缩。
- **紧凑布局**：
  - 3节点时间轴 `.tl-node` 垂直 padding 推荐 **`1.8vh 0`** 左右，行间 gap 推荐 **`2vh`**。
  - 双轨对比底部的 KPI 卡片 padding 推荐 **`1.6vh 2vw`** 左右，大字字号采用 **`min(2.5vw, 4.5vh)`**。
