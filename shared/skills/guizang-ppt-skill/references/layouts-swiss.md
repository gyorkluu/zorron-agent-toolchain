...
**画布**
- `.canvas-card`:`100vw × 100vh`,直角无圆角,padding `5.6vh 5vw 9.6vh`
...
`.canvas-card` 已自带 `padding:5.6vh 5vw 9.6vh`。
...
例外:`.slide.split .canvas-card{padding:0}` 已被 CSS 覆盖,split 模式下两个 `.half` 自己控制 padding(常用 `5.6vh 3.6vw 9.6vh`),与本法则不冲突。
...
<section class="slide split" data-animate="split-statement">
  <div class="canvas-card">
    <div class="split-half">
      <!-- 左半 · IKB + ASCII 呼吸场 -->
      <div class="half b-accent" style="padding:5.6vh 3.6vw 9.6vh;justify-content:space-between;position:relative;overflow:hidden">
        <canvas class="ascii-bg" aria-hidden="true"></canvas>
      ...
      <div class="half" style="padding:5.6vh 3.6vw 9.6vh;justify-content:space-between">
...
**关键类**:`.manifesto-top` `.ink-banner-full`(`margin:0 -5vw -9.6vh` 取消父级 padding)
